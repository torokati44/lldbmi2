...
char* StringB::grow(int at_least) {
...
char* StringB::clear(int bytes, int start) {
    if (buffer_array == NULL)
        return NULL;
...
char* StringB::copy(const char* string, int maxBytes) {
...
char* StringB::append(const char* string, int extraBytes) {
    return copyat(buffer_size, string, BIG_LIMIT, extraBytes);
}

// append string to the end of the buffer
// if bytes specified, append at most bytes bytes and terminate string
char* StringB::append(const char c) {
...
// if bytes specified, copy at most bytes bytes and terminate string
char* StringB::copyat(int offset, const char* string, int maxBytes, int extraBytes) {
...
int StringB::sprintf(const char* format, ...) {
    va_list args;
...
int StringB::catsprintf(const char* format, ...) {
    va_list args;
    va_start(args, format);
...
int StringB::vosprintf(int offset, const char* format, va_list args) {
    va_list args_start;
